...
use druid::{AppLauncher, Color, Lens, Rect, WindowDesc, theme};
...
fn build_root_widget() -> impl Widget<HelloState> {
    ...
    let circle_and_rects = Painter::new(|ctx, _data, _env| {
        ...
        ctx.fill(rect1, &Color::rgba8(0x0, 0xff, 0, 50));
    ...
    //     }
    // });
    let text_background = Painter::new(|ctx, _, _| {
        let bounds = ctx.size().to_rect();
        let rect1 = Rect::new(0., 0., bounds.width(), bounds.height());
        ctx.fill(rect1, &Color::rgba8(0x0, 0xff, 0, 50));
    });
    const LIGHTER_GREY: Color = Color::GREEN;
    let label = Label::new(|data: &HelloState, _env: &Env| {
        ...
        .with_text_color(Color::rgba8(0, 0, 0, 255))
